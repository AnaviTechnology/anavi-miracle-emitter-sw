...
// If HOME_ASSISTANT_DISCOVERY is defined, the Anavi Miracle Emitter will
...
// If PUBLISH_CHIP_ID is defined, the Anavi Miracle Emitter will publish
...
void apWiFiCallback(WiFiManager *myWiFiManager)
{
    ...
    drawDisplay("Miracle Emitter", "Please configure", configHelper.c_str(), true);
}
...
void setup()
{
    ...
    drawDisplay("ANAVI", "Miracle Emitter", "Loading...", true);
...
void printLedStatus()
{
    Serial.println("Print LED Status");
    sensor_line1 = "Effect: ";
    Serial.println(sensor_line1);
    sensor_line2 = effectLed1;
                            ...
                            const char *value_template)
{
    ...
    json["device"]["model"] = "ANAVI Miracle Emitter";
...
bool publishLightDiscovery()
{
    ...
    static char topic[48 + sizeof(machineId)];
    snprintf(topic, sizeof(topic),
             "homeassistant/light/%s/config",  machineId);
    ...
    json["name"] = String(ha_name) + String(" ANAVI Miracle Emitter");
    json["unique_id"] = String("anavi-miracle-emitter") + machineId;
    json["command_topic"] = String("cmnd/") + machineId + String("/color");
    json["state_topic"] = String("stat/") + machineId + String("/led") + String("/#");
